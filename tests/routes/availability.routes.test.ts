...
import { CreateAvailabilityRuleDto, CreateAvailabilityOverrideDto, UpdateAvailabilityOverrideDto } from '../../src/dtos/availability.validation';
// *** CORRECTION 1 : Supprimer AuthResult non exporté ***
...
// *** CORRECTION 2 : Laisser TypeScript inférer le type des agents ***
let clientAgent: any; // Ou let clientAgent;
let ownerAgent: any;  // Ou let ownerAgent;
let otherOwnerAgent: any; // Ou let otherOwnerAgent;
...
const targetDateStr = '2024-07-15'; // Lundi (pour calcul dispo)
let targetDayOfWeek: number; // Sera calculé dans beforeEach

// --- Helpers ---
const expectSlots = (baseDateStr: string, timeSlots: string[]): string[] => {
    return timeSlots.map(time => `${baseDateStr}T${time}.000Z`);
};
const createRule = async (estabId: number, day: number, start: string, end: string) => {
    return await db.AvailabilityRule.create({ establishment_id: estabId, day_of_week: day, start_time: start, end_time: end });
};
const createOverride = async (estabId: number, startISO: string, endISO: string, available: boolean) => {
    return await db.AvailabilityOverride.create({ establishment_id: estabId, start_datetime: new Date(startISO), end_datetime: new Date(endISO), is_available: available });
};
const createBooking = async (svcId: number, userId: number, estabId: number, startISO: string, endISO: string, status: BookingStatus = BookingStatus.CONFIRMED) => {
    await db.Booking.create({
        user_id: userId, establishment_id: estabId, service_id: svcId,
        start_datetime: new Date(startISO), end_datetime: new Date(endISO),
...
describe('Availability Routes Integration Tests (CRUD & Calculation)', () => {
    ...
    beforeAll(async () => {
        try {
            ...
            console.log('Test Database connection authenticated for Availability tests.');
            ...
            console.log('Required roles checked/created for Availability tests.');
        } catch (error) {
            console.error('!!! AVAILABILITY TEST DATABASE SETUP FAILED !!!', error);
            throw error;
        }
    });
    ...
    beforeEach(async () => {
        ...
        // *** CORRECTION 3 : Utiliser 'any' pour authResult ***
        let authResult: any;
        ...
        serviceId_60min = svc60.id;
        serviceId_90min = svc90.id;
        serviceId_30min = svc30.id;
    });
    ...
    // --- CRUD Tests (adaptés aux nouvelles routes) ---
    describe('Availability Admin Routes (CRUD)', () => {
        ...
        describe('GET /api/users/me/establishments/:establishmentId/availability/rules', () => {
            ...
            beforeEach(async () => {
                ...
                await createRule(ownedEstablishment.id, 1, '09:00:00', '17:00:00'); // Mon
                await createRule(ownedEstablishment.id, 3, '10:00:00', '18:00:00'); // Wed
                await createRule(otherEstablishment.id, 1, '08:00:00', '16:00:00'); // Other owner's rule
            });
            it('should return all rules for the owned establishment (200)', async () => {
                ...
                expect(response.body[0].day_of_week).toBe(1);
                expect(response.body[1].day_of_week).toBe(3);
            });
            // Ajouter tests 401, 403, 404, etc.
            it('should return 403 if user is not admin', async () => {
        ...
        describe('DELETE /api/availability/rules/:ruleId', () => {
            ...
            it('should delete the rule successfully if user owns it (204)', async () => {
                ...
                const dbRule = await db.AvailabilityRule.findByPk(existingRuleId);
        ...
        describe('POST /api/users/me/establishments/:establishmentId/availability/overrides', () => {
            const startDate = new Date(`${targetDateStr}T10:00:00.000Z`);
            const endDate = new Date(`${targetDateStr}T14:00:00.000Z`);
            const validOverrideData = { start_datetime: startDate.toISOString(), end_datetime: endDate.toISOString(), is_available: false, reason: "Maintenance" };
            let route: string;
            ...
            it('should create a new override for the owned establishment (201)', async () => {
                ...
                expect(response.status).toBe(201);
                expect(new Date(response.body.start_datetime)).toEqual(startDate);
            });
            it('should return 401 if not authenticated', async () => {
                const response = await supertest(app).post(route).send(validOverrideData);
                expect(response.status).toBe(401);
            });
            it('should return 403 if user is not admin', async () => {
                const response = await clientAgent.post(route).set('Authorization', `Bearer ${clientAccessToken}`).send(validOverrideData);
                expect(response.status).toBe(403);
            });
            ...
            it('should return 400 if body is invalid', async () => {
                const invalidData = { ...validOverrideData, start_datetime: 'invalid-date' };
                const response = await ownerAgent.post(route).set('Authorization', `Bearer ${ownerAccessToken}`).send(invalidData);
                expect(response.status).toBe(400);
            });
        });
        ...
        describe('GET /api/users/me/establishments/:establishmentId/availability/overrides', () => {
            ...
            beforeEach(async () => {
                ...
                const ov1DateStart = new Date(); ov1DateStart.setUTCDate(1); ov1DateStart.setUTCHours(9,0,0,0);
                const ov1DateEnd = new Date(ov1DateStart); ov1DateEnd.setUTCHours(12,0,0,0);
                const ov2DateStart = new Date(); ov2DateStart.setUTCDate(5); ov2DateStart.setUTCHours(0,0,0,0);
                const ov2DateEnd = new Date(ov2DateStart); ov2DateEnd.setUTCDate(6); ov2DateEnd.setUTCHours(0,0,0,0);
                await createOverride(ownedEstablishment.id, ov1DateStart.toISOString(), ov1DateEnd.toISOString(), false);
                await createOverride(ownedEstablishment.id, ov2DateStart.toISOString(), ov2DateEnd.toISOString(), true);
                await createOverride(otherEstablishment.id, ov1DateStart.toISOString(), ov1DateEnd.toISOString(), false);
            });
        ...
        describe('PUT /api/availability/overrides/:overrideId', () => {
            const ovDateStart = new Date(`${targetDateStr}T09:00:00.000Z`);
            const ovDateEnd = new Date(`${targetDateStr}T12:00:00.000Z`);
            const updatedReason = "Updated Reason";
            const updateData = { reason: updatedReason };
            beforeEach(async () => {
                const ov1 = await createOverride(ownedEstablishment.id, ovDateStart.toISOString(), ovDateEnd.toISOString(), false);
                const ov2 = await createOverride(otherEstablishment.id, ovDateStart.toISOString(), ovDateEnd.toISOString(), false);
                existingOverrideId = ov1.id;
                otherOwnerOverrideId = ov2.id;
            });
            it('should update the override successfully if user owns it (200)', async () => {
                const response = await ownerAgent.put(`/api/availability/overrides/${existingOverrideId}`).set('Authorization', `Bearer ${ownerAccessToken}`).send(updateData);
                expect(response.status).toBe(200);
                expect(response.body.reason).toBe(updatedReason);
            });
            it('should return 403 if user does not own the override', async () => {
                const response = await ownerAgent.put(`/api/availability/overrides/${otherOwnerOverrideId}`).set('Authorization', `Bearer ${ownerAccessToken}`).send(updateData);
                expect(response.status).toBe(403);
            });
            it('should return 401 if not authenticated', async () => {
                const response = await supertest(app).put(`/api/availability/overrides/${existingOverrideId}`).send(updateData);
                expect(response.status).toBe(401);
            });
            it('should return 403 if user is not admin', async () => {
                const response = await clientAgent.put(`/api/availability/overrides/${existingOverrideId}`).set('Authorization', `Bearer ${clientAccessToken}`).send(updateData);
                expect(response.status).toBe(403);
            });
            it('should return 404 if override ID does not exist', async () => {
                const response = await ownerAgent.put('/api/availability/overrides/99999').set('Authorization', `Bearer ${ownerAccessToken}`).send(updateData);
                expect(response.status).toBe(404);
            });
            it('should return 400 if body is invalid (empty)', async () => {
                const response = await ownerAgent.put(`/api/availability/overrides/${existingOverrideId}`).set('Authorization', `Bearer ${ownerAccessToken}`).send({});
                expect(response.status).toBe(400);
            });
        });
        ...
        describe('DELETE /api/availability/overrides/:overrideId', () => {
            ...
            const ovDateEnd = new Date(`${targetDateStr}T12:00:00.000Z`);
            beforeEach(async () => {
                const ov1 = await createOverride(ownedEstablishment.id, ovDateStart.toISOString(), ovDateEnd.toISOString(), true);
                const ov2 = await createOverride(otherEstablishment.id, ovDateStart.toISOString(), ovDateEnd.toISOString(), true);
                existingOverrideId = ov1.id;
                otherOwnerOverrideId = ov2.id;
            });
            it('should delete the override successfully if user owns it (204)', async () => {
                ...
                expect(response.status).toBe(204);
                const dbOverride = await db.AvailabilityOverride.findByPk(existingOverrideId);
                expect(dbOverride).toBeNull();
            });
            it('should return 403 if user does not own the override', async () => {
